...
cmd_merge() {
  ...
  # Switch to base branch in main worktree (stdout→stderr to keep eval-safe output)
  git -C main checkout "$base_branch" >&2

  # Remove untracked files in main/ that exist in the incoming plan branch.
  ...
  if [ -n "$incoming_files" ]; then
    while IFS= read -r file; do
      if [ -f "main/$file" ] && ! git -C main ls-files --error-unmatch "$file" &>/dev/null 2>&1; then
        rm -f "main/$file"
  ...
  if ! git -C main merge "$branch_name" --no-edit -X theirs >&2; then
